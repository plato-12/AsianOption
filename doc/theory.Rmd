...
1. For $i = 1, \ldots, M$ (simulations):
   - Generate random path: $\omega^{(i)} \sim \text{Bernoulli}(p^{adj})^n$
   - Compute geometric average: $G(\omega^{(i)})$
   - Calculate payoff: $\pi^{(i)} = \max(0, G(\omega^{(i)}) - K)$

2. Estimate price: $\hat{V}_0 = \frac{1}{r^n} \cdot \frac{1}{M} \sum_{i=1}^M \pi^{(i)}$
...
The output includes:
- **Price**: Estimated option value
- **Std Error**: Standard error of the estimate
- **95% CI**: Confidence interval for the true price
- **Simulations**: Number of paths simulated
