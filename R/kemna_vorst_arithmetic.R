...
#'   estimated option price, as a time-0 present value). If \code{return_diagnostics = TRUE}, returns a list with components:
#'   \describe{
...
#'     \item{geometric_price}{Analytical geometric average price used as the
#'       control variate, discounted to time 0 on the same basis as the
#'       simulated payoffs}
...
#'
#' @export
price_kemna_vorst_arithmetic <- function(S0, K, r, sigma, T0, T_mat, n, M = 10000,
                                          option_type = "call",
  ...
  result <- price_kemna_vorst_arithmetic_cpp(
    ...
    T0 = T0, T_mat = T_mat, n = as.integer(n), M = as.integer(M),
...
#' @param ... Additional arguments (ignored)
#'
#' @return Invisibly returns the input object \code{object}. Called for side effects (printing).
#'
#' @export
