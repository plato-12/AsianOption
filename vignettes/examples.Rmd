...
## Example 8: Convergence Analysis (Exact vs Monte Carlo)
...
**Observation**: The transition from exact to Monte Carlo is seamless, with
Monte Carlo enabling pricing for much larger n values that would be infeasible
with exact enumeration.

## Example 8b: Monte Carlo Accuracy Validation

Validate Monte Carlo against exact solution:
...
## Example 8c: Large n Pricing (Monte Carlo Only)

Price options with very large n (impossible with exact method):
...
8. **Convergence behavior with exact and Monte Carlo methods**
   - Automatic method selection
   - Monte Carlo accuracy validation
   - Large n pricing (n=50, 75, 100)
...
**Key Insights**:
- **Exact method**: Optimal for n ≤ 20 (exact, fast)
- **Monte Carlo**: Essential for n > 20 (fast, accurate)
- **Automatic selection**: Package seamlessly chooses the best method
- **Accuracy**: MC typically achieves < 0.5% error with 100,000 simulations
