...
through detailed examples. All examples use verified code that has been tested
to work correctly.
...
all $2^n$ paths. No sampling or approximation error is present.
...
1. **Basic pricing**: Geometric Asian options with and without price impact
2. **Parameter sensitivity**: Effects of $\lambda$, strike, volumes
3. **Option types**: Calls, puts, European, Asian
4. **Arithmetic bounds**: Global and path-specific bounds
5. **Comparative analysis**: Impact vs no impact, European vs Asian
6. **Advanced topics**: Asymmetric hedging, moneyness, convergence

**Key Insights**:

- **Exact computation**: All prices use complete path enumeration (no sampling)
- **Efficiency**: Optimal for n ≤ 20 (fast and exact)
- **Price impact**: Systematically increases option values
- **Path-specific bounds**: Provide much tighter estimates than global bounds
- **Flexibility**: Supports calls, puts, symmetric and asymmetric hedging
