...
#' @param lambda_bar_T Transient impact coefficient.
#' @param lambda_bar_P Permanent impact coefficient.
#' @param k_A Execution cost coefficient (buy).
#' @param k_B Execution cost coefficient (sell).
#' @param psi_cost Cost exponent in (0, 1].
#' @param eta Order-flow noise amplitude (scalar or vector of length N).
#' @param p Binomial probability in (0, 1).
#' @param I0 Initial impact state.
#' @return Invisible NULL on success; stops with an error otherwise.
#' @keywords internal
validate_hjb_inputs <- function(S0, K, T, N, sigma, r_cont,
  ...
  if (alpha_m < 0) {
    warning(sprintf(
      paste0("alpha_m = 1 - kappa*dt = %.4f < 0. ",
             "kappa may be too large for N = %d. ",
